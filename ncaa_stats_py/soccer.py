import asyncio
async def async_get_soccer_team_schedule(team_id: int) -> pd.DataFrame:
	"""
	Async version of get_soccer_team_schedule. Use this in async environments (Jupyter, FastAPI, etc).
	"""
	...
	if load_from_cache is True:
		return games_df
	try:
		response = await _get_webpage_async(url, wait_for_selector='table')
		if not response:
			logging.error(f"_get_webpage_async returned None for url: {url}")
			return games_df
		if not response.text:
			logging.error(f"_get_webpage_async returned empty text for url: {url}")
			return games_df
	except Exception as e:
		import traceback
		logging.error(f"Exception in _get_webpage_async for url {url}: {e}\n{traceback.format_exc()}")
		return games_df
...
def get_soccer_team_schedule(team_id: int) -> pd.DataFrame:
	...
	# Synchronous version: only safe outside of async event loops.
	try:
		asyncio.get_running_loop()
		raise RuntimeError("get_soccer_team_schedule cannot be called from an async event loop. Use async_get_soccer_team_schedule instead.")
	except RuntimeError:
		pass
	# ...existing code for sync version...
	sport_id = "WSO"
	schools_df = _get_schools()
	games_df = pd.DataFrame()
	games_df_arr = []
